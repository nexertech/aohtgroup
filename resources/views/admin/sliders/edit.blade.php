            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <form action="{{ route('admin.sliders.update', $slider->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            @if($slider->image)
                                <div class="mb-3">
                                    <img src="{{ asset('storage/' . $slider->image) }}" alt="Current Slider"
                                        class="w-48 h-24 object-cover rounded border border-gray-200">
                                    <p class="mt-1 text-xs text-gray-500">Current image (will be replaced if you upload a new
                                        one)
                                    </p>
                                </div>
                            @endif
                        ...
                        <div>
                            <label for="button_text" class="block text-sm font-medium text-gray-700 mb-2">Button
                                Text</label>
                        ...
                        <div>
                            <label for="button_link" class="block text-sm font-medium text-gray-700 mb-2">Button
                                Link</label>
                        ...
                        <div>
                            ...
                            <input type="number" name="sequence" id="sequence"
                                value="{{ old('sequence', $slider->sequence) }}"
                                class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                                required>
                            @error('sequence')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
