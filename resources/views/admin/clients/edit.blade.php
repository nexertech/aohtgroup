    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden p-6 relative">
            ...
            <form action="{{ route('admin.clients.update', $client) }}" method="POST" enctype="multipart/form-data"
                ...
                <!-- Name Field -->
                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-700 mb-1">Client Name <span
                ...
                <!-- URL Field -->
                <div>
                    <label for="url" class="block text-sm font-semibold text-gray-700 mb-1">Website URL <span
                            class="text-gray-400 font-normal">(Optional)</span></label>
                ...
                <!-- Logo Field -->
                <div>
                    <label for="logo" class="block text-sm font-semibold text-gray-700 mb-1">Client Logo</label>
                    <div
                ...
                <div id="imagePreviewContainer" class="{{ $client->logo ? '' : 'hidden' }} mt-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Current/New Logo</label>
                    <div class="relative inline-block">
                        <img id="imagePreview" src="{{ $client->logo ? asset($client->logo) : '#' }}" alt="Preview"
                            class="h-32 w-auto rounded-lg border border-gray-200 shadow-sm object-contain bg-gray-50 p-2">
                        <!-- We don't necessarily want to remove the existing image from DB on 'x' click here, usually just resetting input.
                                 For simplicity, we'll just allow clearing the new input, but if it was an existing one, it remains visual until saved.
                                 Actually, let's keep it simple: visual preview only. -->
                    </div>
                </div>
