    ...
    <div class="container-fluid p-6">
        <div class="max-w-4xl mx-auto">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-semibold text-gray-800">Edit Application</h1>
                ...
                <form action="{{ route('admin.job-applications.update', $jobApplication->id) }}" method="POST"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $jobApplication->name) }}"
                        ...
                        <div>
                            <label for="email" class="block text-sm font-medium text-gray-700 mb-2">Email <span
                                    class="text-red-500">*</span></label>
                            <input type="email" name="email" id="email" value="{{ old('email', $jobApplication->email) }}"
                                class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm"
                                required>
                            @error('email')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div>
                            ...
                            <input type="text" name="phone" id="phone" value="{{ old('phone', $jobApplication->phone) }}"
                                class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            @error('phone')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="md:col-span-2">
                            ...
                            @if($jobApplication->cv_file)
                                <div class="mb-2 text-sm text-green-600 flex items-center">
                                    ...
                                    Current file: <a href="{{ asset('storage/' . $jobApplication->cv_file) }}" target="_blank"
                                        class="underline ml-1">View CV</a>
                                </div>
                            @endif
                            <input type="file" name="cv_file" id="cv_file"
                                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100">
                            <p class="mt-1 text-xs text-gray-500">Upload new file to replace current one.</p>
                            @error('cv_file')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="cover_letter" id="cover_letter" rows="5"
                                class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">{{ old('cover_letter', $jobApplication->cover_letter) }}</textarea>
                            @error('cover_letter')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
